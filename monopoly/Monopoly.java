...
public class Monopoly {
	...
	// Ajoute des joueurs avec une error handler
	...
	private void jouerTour(Joueur j) {
		int de1 = lancerDe();
		int de2 = lancerDe();
		int totalDe = de1 + de2;
	...
	private void acheterPropriete(Joueur j, Propriete p) {
	...
	private void payerLoyer(Joueur j, Propriete p) {
	...
	// Propriete p = (Propriete) c;
	// if (p.getProprietaire() == null) {
	// System.out.print(j.getNom() + " veut-il acheter la "
	// + p.getNom() + " (" + p.getPrix()
	// + " euros) (o-n)? ");
	// String reponse = scanner.nextLine().toLowerCase();
	// if (reponse.equals("o")) {
	...
	// j.retirerArgent(p.getLoyer());
	// p.getProprietaire().ajouterArgent(p.getLoyer());
	// System.out.println(j.getNom() + " a payé " + p.getLoyer()
	// + " à " + p.getProprietaire().getNom());
	// }
	// } else if (c instanceof Chance) {
	// Chance chance = (Chance) c;
	// System.out.print("Tirage d'une carte chance : ");
	...
	// j.ajouterArgent(montant);
	// System.out.println(
	// j.getNom() + " reçoit " + montant + " euros");
	// } else {
	// j.retirerArgent(-montant);
	// System.out.println(j.getNom() + " doit payer " + (-montant)
	// + " euros");
	// }
	// } else if (c instanceof AllerPrison) {
	// j.setPosition(10);
